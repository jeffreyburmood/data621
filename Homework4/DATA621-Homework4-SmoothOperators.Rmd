...
model_3 <- glm(TARGET_FLAG ~ .-TARGET_AMT-OLDCLAIM,family=binomial(link='logit'),data=train)
model_3_backward <- step(model_3)
summary(model_3_backward)

p <- predict(model_3_backward, newdata=subset(test,select=c(3:cols)), type="response")
pr <- prediction(p, test$TARGET_FLAG)
prf <- performance(pr, measure = "tpr", x.measure = "fpr")
plot(prf)
abline(a = 0, b = 1)
...
p <- predict(model_3_backward, newdata=eval_copy, type = "response")
