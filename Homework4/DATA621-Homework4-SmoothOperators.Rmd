...
ins <- read.csv(text=getURL("https://raw.githubusercontent.com/jeffreyburmood/data621/master/Homework4/data/insurance_training_data.csv"),header=TRUE,na.strings=c(" "))
ins_eval <- read.csv(text=getURL("https://raw.githubusercontent.com/jeffreyburmood/data621/master/Homework4/data/insurance-evaluation-data.csv"),header=TRUE,na.strings=c(" "))
...
insc$CAR_AGE[insc$CARE_AGE < 0 ] <- 0  #If car age is below zero, set to zero
...
print("Summary for insi")
summary(insi)


#use the Amelia II package to create the imputations
...
ins <- ins[2:length(ins)]
# Next, get a general look at the data
# Let's start by exploring the type of each variable
types <- sapply(1:length(ins),function(x) typeof(ins[,x]))
types.df <- data.frame(VAR=names(ins),TYPE=types)
#kable(types.df)
# Now generate some summary statistics
kable(summary(ins[1:6]))
kable(summary(ins[7:12]))
kable(summary(ins[13:18]))
kable(summary(ins[19:length(ins)]))
