...
For regular negative binomial model, we start with all the dependent variables, and perform backward stepwise algorythm. Initially, we have 14 dependent variables, we get it down to 10 variables. The AIC is 37306.  
...
Lastly we are going to look at a regular linear model, as a comparison to the data shown above. Again we are going to compare Chemistry vs. Perception, to see if there is any relationship between chemisty. 
...
We can see not all the chemistry variables are significant, using backward stepwise, we can narrow down the insignificant independent variables. This produced the following forumula for  
...
From the the models, we can clearly see that the perception data was a much more appropriate fit. This can be seen through the R-Squared value, which shows that the perception model explains roughly 50% of the variance in the model. This is a pretty "good-fit." Using the chemistry data, we also se a significant model, however, the fit is much worse, with practical none of the variance explained. 
...
From the above, we can clearly see that the first chemistry model shows clear patterns in the residuals, which indicates that linear modeling is not at all a good choice for those particular variables. However, just using the perception variables we see a much better picture, with a more uniformed residual distribution, and no clear patterns that would suggest another choice in models. 

##Select Models  
  
  
When we compare all our regular models first, we built a dataframe which contains all the performance parameter of the models. Out of the four regular models, it is easy to notice that regular linear model with focuse on perception is the best model. It has the lowest AIC and BIC. The Log-Likelihood is also the highest among the four. 
...
When we compare two zero-inflated models, the following code is telling us that zero inflated poisson model might be a better model, althought the statistics does not show the differences are statistically significant. But zero inflated poisson model still has one lower degree of freedom. 
```{r}
#Compare two zero-inflated models
lmtest::lrtest(zn.simplified, zp.simplified)
```
  
By comparing regular linear model with focus on perception with zero inflated poisson model, we end up with the model we want to use for our final evaluation. 
```{r}
#Compare zero inflated poisson model with regular linear model perception focused. 
...
This is our final predicted results.   
```{r}
