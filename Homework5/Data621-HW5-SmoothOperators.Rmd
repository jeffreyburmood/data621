...
For regular negative binomial model, we start with all the dependent variables, and perform a backward stepwise algorithm. Initially, we have 14 dependent variables; using this process we reduce to 10 variables. The AIC is 37306.  
...
When we compare the two zero-inflated models, the following code tells us that zero inflated poisson model might be a better model, although the statistics do not show the differences are significant. But the zero inflated poisson model has one lower degree of freedom. 
```{r}
#Compare two zero-inflated models
lmtest::lrtest(zn.simplified, zp.simplified)
```
  
By comparing the regular linear model with focus on perception to the zero inflated poisson model, we end up with the model we want to use for our final evaluation: 
```{r}
